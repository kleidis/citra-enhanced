...
GMainWindow::GMainWindow(Core::System& system_)
    ...
    if (gl_renderer.startsWith(QStringLiteral("D3D12"))) {
        ...
        // We can override the version here to allow Mandarin to work.
...
#ifdef MANDARIN_ENABLE_COMPATIBILITY_REPORTING
...
void GMainWindow::InitializeHotkeys() { // TODO: This code kind of sucks
    ...
    link_action_shortcut(ui->action_Exit, QStringLiteral("Exit Mandarin"));
...
void GMainWindow::ConnectMenuEvents() {
    ...
    // Help
    connect_menu(ui->action_Open_Mandarin_Folder, &GMainWindow::OnOpenMandarinFolder);
...
static std::optional<QDBusObjectPath> HoldWakeLockLinux(u32 window_id = 0) {
    ...
    QVariantMap options = {};
    //: TRANSLATORS: This string is shown to the user to explain why Mandarin needs to prevent the
    //: computer from sleeping
    options.insert(QString::fromLatin1("reason"),
                   QCoreApplication::translate("GMainWindow", "Mandarin is running a game"));
    ...
    if (result != Core::System::ResultStatus::Success) {
        ...
        case Core::System::ResultStatus::ErrorLoader_ErrorGbaTitle:
            QMessageBox::critical(this, tr("Unsupported ROM"),
                                  tr("GBA Virtual Console ROMs are not supported by Mandarin."));
            break;
...
bool GMainWindow::MakeShortcutIcoPath(const u64 program_id, const std::string_view game_file_name,
    ...
    // Get path to Mandarin icons directory & icon extension
...
void GMainWindow::OnGameListCreateShortcut(u64 program_id, const std::string& game_path,
    ...
    const std::string comment = fmt::format("Start {:s} with the Mandarin Emulator", game_title);
    ...
    case Service::AM::InstallStatus::ErrorEncrypted:
        QMessageBox::critical(this, tr("Encrypted File"),
                              tr("%1 must be decrypted "
                                 "before being used with Mandarin. A real 3DS is required.")
                                  .arg(filename));
        break;
...
void GMainWindow::OnMenuReportCompatibility() {
    if (!NetSettings::values.mandarin_token.empty() && !NetSettings::values.mandarin_username.empty()) {
        ...
        QMessageBox::critical(this, tr("Missing Mandarin Account"),
                              tr("You must link your Mandarin account to submit test cases."
...
void GMainWindow::OnOpenMandarinFolder() {
...
void GMainWindow::OnMenuAboutMandarin() {
...
bool GMainWindow::ConfirmChangeGame() {
    ...
    auto answer = QMessageBox::question(
        this, tr("Mandarin"), tr("The game is still running. Would you like to stop emulation?"),
        QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
    return answer != QMessageBox::No;
}
...
void GMainWindow::UpdateWindowTitle() {
    ...
    if (game_title_long.isEmpty()) {
        ...
        setWindowTitle(QStringLiteral("Mandarin %1 | %2").arg(full_name, game_title_long));
...
int main(int argc, char* argv[]) {
    ...
    QCoreApplication::setOrganizationName(QStringLiteral("Mandarin team"));
    QCoreApplication::setApplicationName(QStringLiteral("Mandarin"));
