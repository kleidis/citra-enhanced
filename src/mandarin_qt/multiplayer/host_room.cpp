...
HostRoomWindow::HostRoomWindow(Core::System& system_, QWidget* parent, QStandardItemModel* list,
    ...
    if (ui->username->text().isEmpty() && !NetSettings::values.mandarin_username.empty()) {
        // Use Mandarin Web Service user name as nickname by default
        ui->username->setText(QString::fromStdString(NetSettings::values.mandarin_username));
    }
    ...
    if (auto member = Network::GetRoomMember().lock()) {
        ...
        if (auto room = Network::GetRoom().lock()) {
            bool created = room->Create(ui->room_name->text().toStdString(),
                                        ...
                                        NetSettings::values.mandarin_username, game_name.toStdString(),
                ...
                if (result.result_code != Common::WebResult::Code::Success) {
                    QMessageBox::warning(
                        ...
                        tr("Failed to announce the room to the public lobby. In order to host a "
                           "room publicly, you must have a valid Mandarin account configured in "
