...
get_addr_fp_t GetAddressingOpLoadStoreT(unsigned int inst) {
    ...
    // So it's sufficient for mandarin to not implement this.
    return nullptr;
}
...
static int InterpreterTranslateBlock(ARMul_State* cpu, std::size_t& bb_start, u32 addr) {
    MANDARIN_PROFILE("DynCom", "Decode");
...
unsigned InterpreterMainLoop(ARMul_State* cpu) {
    MANDARIN_PROFILE("DynCom", "Execute");
...
BXJ_INST : {
    ...
    //
    // This is sufficient for mandarin, as the CPU for the 3DS does not implement Jazelle.
