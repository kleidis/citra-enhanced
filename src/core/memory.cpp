...
private:
    bool* At(VAddr addr) {
        if (addr >= VRAM_VADDR && addr < VRAM_VADDR_END) {
            return &vram[(addr - VRAM_VADDR) / MANDARIN_PAGE_SIZE];
        }
        if (addr >= LINEAR_HEAP_VADDR && addr < LINEAR_HEAP_VADDR_END) {
            return &linear_heap[(addr - LINEAR_HEAP_VADDR) / MANDARIN_PAGE_SIZE];
        }
        if (addr >= NEW_LINEAR_HEAP_VADDR && addr < NEW_LINEAR_HEAP_VADDR_END) {
            return &new_linear_heap[(addr - NEW_LINEAR_HEAP_VADDR) / MANDARIN_PAGE_SIZE];
        }
        if (addr >= PLUGIN_3GX_FB_VADDR && addr < PLUGIN_3GX_FB_VADDR_END) {
            return &plugin_fb[(addr - PLUGIN_3GX_FB_VADDR) / MANDARIN_PAGE_SIZE];
        }
        return nullptr;
    }

    std::array<bool, VRAM_SIZE / MANDARIN_PAGE_SIZE> vram{};
    std::array<bool, LINEAR_HEAP_SIZE / MANDARIN_PAGE_SIZE> linear_heap{};
    std::array<bool, NEW_LINEAR_HEAP_SIZE / MANDARIN_PAGE_SIZE> new_linear_heap{};
    std::array<bool, PLUGIN_3GX_FB_SIZE / MANDARIN_PAGE_SIZE> plugin_fb{};
...
public:
    ...
    void ReadBlockImpl(const Kernel::Process& process, const VAddr src_addr, void* dest_buffer,
        ...
        std::size_t page_index = src_addr >> MANDARIN_PAGE_BITS;
        std::size_t page_offset = src_addr & MANDARIN_PAGE_MASK;

        while (remaining_size > 0) {
            const std::size_t copy_amount = std::min(MANDARIN_PAGE_SIZE - page_offset, remaining_size);
            const auto current_vaddr =
                static_cast<VAddr>((page_index << MANDARIN_PAGE_BITS) + page_offset);
    ...
    void WriteBlockImpl(const Kernel::Process& process, const VAddr dest_addr,
        ...
        auto& page_table = *process.vm_manager.page_table;
        std::size_t remaining_size = size;
        std::size_t page_index = dest_addr >> MANDARIN_PAGE_BITS;
        std::size_t page_offset = dest_addr & MANDARIN_PAGE_MASK;
...
void MemorySystem::MapPages(PageTable& page_table, u32 base, u32 size, MemoryRef memory,
    ...
    LOG_DEBUG(HW_Memory, "Mapping {} onto {:08X}-{:08X}", (void*)memory.GetPtr(),
              base * MANDARIN_PAGE_SIZE, (base + size) * MANDARIN_PAGE_SIZE);

    if (impl->system.IsPoweredOn()) {
        RasterizerFlushVirtualRegion(base << MANDARIN_PAGE_BITS, size * MANDARIN_PAGE_SIZE,
    ...
    while (base != end) {
        ...
        if (type == PageType::Memory && impl->cache_marker.IsCached(base * MANDARIN_PAGE_SIZE)) {
            page_table.attributes[base] = PageType::RasterizerCachedMemory;
            page_table.pointers[base] = nullptr;
        }

        base += 1;
        if (memory != nullptr && memory.GetSize() > MANDARIN_PAGE_SIZE)
            memory += MANDARIN_PAGE_SIZE;
    }
}

void MemorySystem::MapMemoryRegion(PageTable& page_table, VAddr base, u32 size, MemoryRef target) {
    ASSERT_MSG((size & MANDARIN_PAGE_MASK) == 0, "non-page aligned size: {:08X}", size);
    ASSERT_MSG((base & MANDARIN_PAGE_MASK) == 0, "non-page aligned base: {:08X}", base);
    MapPages(page_table, base / MANDARIN_PAGE_SIZE, size / MANDARIN_PAGE_SIZE, target, PageType::Memory);
}

void MemorySystem::UnmapRegion(PageTable& page_table, VAddr base, u32 size) {
    ASSERT_MSG((size & MANDARIN_PAGE_MASK) == 0, "non-page aligned size: {:08X}", size);
    ASSERT_MSG((base & MANDARIN_PAGE_MASK) == 0, "non-page aligned base: {:08X}", base);
    MapPages(page_table, base / MANDARIN_PAGE_SIZE, size / MANDARIN_PAGE_SIZE, nullptr,
             PageType::Unmapped);
}
...
T MemorySystem::Read(const VAddr vaddr) {
    const u8* page_pointer = impl->current_page_table->pointers[vaddr >> MANDARIN_PAGE_BITS];
    if (page_pointer) {
        // NOTE: Avoid adding any extra logic to this fast-path block
        T value;
        std::memcpy(&value, &page_pointer[vaddr & MANDARIN_PAGE_MASK], sizeof(T));
        return value;
    }
    ...
    PageType type = impl->current_page_table->attributes[vaddr >> MANDARIN_PAGE_BITS];
    switch (type) {
    case PageType::Unmapped:
...
void MemorySystem::Write(const VAddr vaddr, const T data) {
    u8* page_pointer = impl->current_page_table->pointers[vaddr >> MANDARIN_PAGE_BITS];
    if (page_pointer) {
        // NOTE: Avoid adding any extra logic to this fast-path block
        std::memcpy(&page_pointer[vaddr & MANDARIN_PAGE_MASK], &data, sizeof(T));
        return;
    }
...
bool MemorySystem::WriteExclusive(const VAddr vaddr, const T data, const T expected) {
    ...
    if (page_pointer) {
        const auto volatile_pointer =
            reinterpret_cast<volatile T*>(&page_pointer[vaddr & MANDARIN_PAGE_MASK]);
...
bool MemorySystem::IsValidVirtualAddress(const Kernel::Process& process, const VAddr vaddr) {
    ...
    auto page_pointer = page_table.pointers[vaddr >> MANDARIN_PAGE_BITS];
    if (page_pointer) {
        return true;
    }

    if (page_table.attributes[vaddr >> MANDARIN_PAGE_BITS] == PageType::RasterizerCachedMemory) {
        return true;
    }
...
u8* MemorySystem::GetPointer(const VAddr vaddr) {
    u8* page_pointer = impl->current_page_table->pointers[vaddr >> MANDARIN_PAGE_BITS];
    if (page_pointer) {
        return page_pointer + (vaddr & MANDARIN_PAGE_MASK);
    }

    if (impl->current_page_table->attributes[vaddr >> MANDARIN_PAGE_BITS] ==
...
void MemorySystem::RasterizerMarkRegionCached(PAddr start, u32 size, bool cached) {
    ...
    u32 num_pages = ((start + size - 1) >> MANDARIN_PAGE_BITS) - (start >> MANDARIN_PAGE_BITS) + 1;
    PAddr paddr = start;

    for (unsigned i = 0; i < num_pages; ++i, paddr += MANDARIN_PAGE_SIZE) {
        for (VAddr vaddr : PhysicalToVirtualAddressForRasterizer(paddr)) {
            impl->cache_marker.Mark(vaddr, cached);
            for (auto& page_table : impl->page_table_list) {
                PageType& page_type = page_table->attributes[vaddr >> MANDARIN_PAGE_BITS];
                ...
                if (cached) {
                    ...
                    case PageType::Memory:
                        ...
                        page_table->pointers[vaddr >> MANDARIN_PAGE_BITS] = nullptr;
                        break;
                    default:
                        UNREACHABLE();
                    }
                    ...
                    case PageType::RasterizerCachedMemory: {
                        ...
                        page_table->pointers[vaddr >> MANDARIN_PAGE_BITS] =
                            GetPointerForRasterizerCache(vaddr & ~MANDARIN_PAGE_MASK);
                        break;
                    }
                    default:
                        UNREACHABLE();
