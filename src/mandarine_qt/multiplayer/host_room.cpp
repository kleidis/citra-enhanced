...
HostRoomWindow::HostRoomWindow(Core::System& system_, QWidget* parent, QStandardItemModel* list,
    ...
    if (ui->username->text().isEmpty() && !NetSettings::values.mandarine_username.empty()) {
        // Use Mandarine Web Service user name as nickname by default
        ui->username->setText(QString::fromStdString(NetSettings::values.mandarine_username));
    }
                ...
                if (result.result_code != Common::WebResult::Code::Success) {
                    QMessageBox::warning(
                        ...
                        tr("Failed to announce the room to the public lobby. In order to host a "
                           "room publicly, you must have a valid Mandarine account configured in "
